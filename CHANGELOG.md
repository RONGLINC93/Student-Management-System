...
- 原「学生管理 → 更多 → 批量导入示例」属于数据维护操作，整体迁移到「**系统设置 → 数据管理 → 示例数据**」，学生管理页「更多」菜单只保留「下载示例表格」「导入表格数据」
- 新入口生成逻辑改为**按当前「考试科目设置」的科目逐科生成**成绩（各科 70~99，逐生 / 逐科错开，修正原实现中所有示例学生成绩完全相同的问题），12 名示例学生覆盖高一 / 高二 / 高三并轮换特长
- 迁移后补充二次确认弹窗，明确提示「追加写入、不会覆盖或删除已有学生、重复导入会产生同学号多条档案」；导入后提示导入人数，仍复用 `POST /api/students/batch`
