...
本版本核心：**「教务角色登录管理后台」**。职位含「教务」（教务 / 教务主任）的教师可凭工号在管理员入口登录后台，管理教务相关项目。
...
- **教务权限范围（服务端硬拦截 + 前端入口适配）**：
  - **可管理**：学生管理、班级、年级、学生回收站、成绩、考勤、操行（德育）、通知公告、宿舍——以上模块读写均可
  - **仅查看**：教师管理、智能分班、大屏直播、备份 / 审计、服务器管理、系统设置（页面 403）、**请假（审批由班主任在教师端完成，教务仅可查看）**；写操作统一返回「教务账号仅可管理…教务项目」提示
- **前端适配**：顶栏账号胶囊显示「教务」徽章；隐藏「系统设置 / 智能分班」入口；教师管理页隐藏「添加教师 / 任班主任 / 编辑删除」按钮；分班大屏隐藏「去分班」；审计日志角色显示「教务」
- 教师管理 / 教师中心表单的「职称 / 职务」新增「教务」「教务主任」选项；`login.html` 与「系统设置 → 账号与安全」补充教务角色说明
