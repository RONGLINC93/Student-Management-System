...
- **API（`/api/teacher/*`，自行鉴权，不走后台权限闸门；与后台 `/api/teachers` 管理接口严格区分）**：
  ...
  - `PUT /api/teacher/profile` 完善个人身份信息（性别 / 任教学科 / 职称 / 联系电话 / 入职年份）
  - `PUT /api/teacher/password` 修改密码（强制改密状态免旧密码）
- **页面**：
  - 新增 `tlogin.html` 教师登录页（紫罗兰主题，含首次登录说明、入口互链）
  - 新增 `teacher.html` 教师中心：个人档案卡片（工号 / 身份证号脱敏 / 学科 / 职称 / 电话 / 入职年份，支持在线「完善 / 编辑资料」）、班主任班级花名册（学号 / 姓名 / 性别 / 特长 / 宿舍分布，支持搜索，含男女统计）、在线修改密码；未登录访问自动跳转登录页，已登录访问 `tlogin.html` 自动进入教师中心
- **审计**：教师端写操作（如完善个人信息）自动记入审计日志（角色 = 教师，排除登录 / 退出 / 改密）

### 变更

- 教师档案新增**身份证号**字段（`idCard`，自动清洗为数字 + 大写 X、限 18 位）：后台「教师管理」表单可录入，表格以脱敏形式展示（前 4 + 后 4），支持按身份证号搜索；教师端仅展示脱敏号码，不可自行修改
